...
## Releases after 2026.09.14

These changes shipped together. **Upgrade the server and the SDK in the same step** — `disc codegen` re-vendors the SDK into `dbschema/disc-client/sdk/` whenever the binary changes, so run it right after installing the new binary.
